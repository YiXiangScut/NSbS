...
//        	solver.add(ctx.mkLe(total_different, ctx.mkInt(d)));  // total difference <= d
    ...
    // Solve a solutions that is different from existing prod with at least  (j-1) * d features and at most j *d features
    public Binary solveDifferentWithD(Product prod, int j, int d) {
        solver.push();
        try {                	
        	Random randomGenerator = new Random();
	       	int randomInt = randomGenerator.nextInt(500000000);
        	...
        	//parseDifference(prod);
        	System.out.println("d= " + d);   
        	System.out.println("(j-1) * d + 1)= " + ((j-1) * d + 1));
        	System.out.println("j*d = " + j*d);
        	
        	//System.out.println(total_selected);
        	
        	solver.add(ctx.mkGe(total_selected, ctx.mkInt((j-1) * d + 1)));  // total difference > (j-1) * d
        	solver.add(ctx.mkLe(total_selected, ctx.mkInt(j*d)));  // total difference <= j * d
