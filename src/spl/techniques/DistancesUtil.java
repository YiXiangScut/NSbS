...
public class DistancesUtil {
    ...
    //Used in sampling products
    public static double getWeightedDistance (Product p1, Product p2) {
  	    Set<Integer> intersection = new HashSet<Integer>(p1);
  	    Set<Integer> union = new HashSet<Integer>(p1);
        intersection.retainAll(p2);     
        //union.addAll(p2);        
        ...
        double delta = 1.0;
        delta = 1.0 / Math.max(p1.getSelectedNumber(), p2.getSelectedNumber());
        
        //System.out.println(p1.getSelectedNumber() +  p2.getSelectedNumber());        
        if (p1.getSelectedNumber() +  p2.getSelectedNumber() > NSbS_Driver.getInstance().numFeatures) {
        	delta = 1.0 / ( NSbS_Driver.getInstance().numFeatures - Math.min(p1.getSelectedNumber(), p2.getSelectedNumber()));
        }
        		
        dist = 0.5 *  (1.0 - intersectionSize / p1.size() ) * delta // 
        	 + 0.5 * ((double)(Math.abs(p1.getSelectedNumber() - p2.getSelectedNumber())) /(NSbS_Driver.getInstance().numFeatures)) ;
        return dist;
