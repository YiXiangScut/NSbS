...
//        
        ...
        for (int i = 0; i < populationSize; i++) {
            Individual indiv = new Individual(NSbS_Driver.getInstance().getUnpredictableProducts(individualSize));
...
//        System.exit(0);
        ...
        while (System.currentTimeMillis() - startTimeMS < timeAllowedMS) {
            ...
            Individual rand = new Individual(NSbS_Driver.getInstance().getUnpredictableProducts(individualSize));
...
//            timeRecord(indiv,sumTimeMS);             
    ...
    public void timeRecord(Individual indiv,long sumTimeMS) {
        ...
        double cov = NSbS_Driver.getInstance().getCoverage(indiv.getProducts());
        runCoverage.add(cov);
    ...
    public void timeRecord(String path, Individual indiv,int recordTimes) throws Exception {
    	...
    	(NSbS_Driver.getInstance()).writeProductsToFile(path + "Products." + recordTimes, indiv.getProducts());      
    ...
    public void runSimpleGA3(List<Product> init) throws Exception {
        ...
        while (a == 1) {
            ...
            prods.addAll(NSbS_Driver.getInstance().getUnpredictableProducts(5000));
